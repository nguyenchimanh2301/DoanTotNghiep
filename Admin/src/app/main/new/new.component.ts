...
//       this.filesv.uploadFile(this.selectedFile).subscribe(
//         response => {
...
export class NewComponent {
  ...
  constructor(private http: HttpClient,private apisv:ApiService) { }
  ...
  uploadFile() {
    if (this.selectedFile) {
      ...
      this.http.post(this.host+'/api/FileUpload', formData)
        .subscribe(
          response => {
            console.log(response); // Xử lý phản hồi từ API (nếu cần)
          },
